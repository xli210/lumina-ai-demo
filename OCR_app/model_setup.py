...
def _read_license() -> str:
    """Read the master key from .machine_lock (online) or .license (legacy)."""
    if os.path.isfile(MACHINE_LOCK_FILE):
        try:
            with open(MACHINE_LOCK_FILE, "r") as f:
                lock = json.load(f)
    ...
    raise RuntimeError(
        "No master key found.\n"
        "Please activate your license first (run launch_app.py and enter your key),\n"
        "or place a master key in .license for standalone use."
    )
...
def download_and_encrypt(
    ...
    Args:
        ...
        license_key:  Encryption key (default: read from .license / .machine_lock)
