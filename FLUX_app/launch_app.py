"""
FLUX.2 Klein Desktop Launcher — one-click start.
...
FLUX_APP_SCRIPT = os.path.join(PROJECT_ROOT, "flux_app.py")
...
def wait_for_runner(callback=None):
    ...
    while time.monotonic() < deadline:
        ...
        if callback:
            callback(f"Loading FLUX model{'.' * dots}  ({elapsed}s)")
...
def run_native_app():
    if not os.path.isfile(FLUX_APP_SCRIPT):
        print("flux_app.py not found, falling back to web mode.")
        return False
...
def run_launcher_gui():
    ...
    def startup():
        try:
            ...
            start_server()
            time.sleep(1)
            root.after(0, lambda: lbl.config(text="Ready. FLUX.2 Klein is running."))
...
def run_launcher_console():
    print("FLUX.2 Klein launcher (console mode)")
