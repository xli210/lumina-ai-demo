"""
FLUX.2 Klein — Native Desktop Application.
...
class FluxApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("FLUX.2 Klein — Image Generator")
    ...
    def _show_activation_dialog(self) -> bool:
        ...
        win.title("Activate FLUX.2 Klein")
        win.minsize(420, 210)
    ...
    def _build_ui(self):
        ...
        ttk.Label(title_frame, text="FLUX.2 Klein", style="Title.TLabel").pack(side=tk.LEFT)
        ...
        def check():
            if runner_healthy():
                ...
                self.after(0, lambda: self._set_status(
                    "Starting FLUX runner (loading model, may take 30-120s)...", "warn"))
                self._start_runner()
            ...
            while time.monotonic() - start < timeout:
                ...
                if dl_status:
                    ...
                    self.after(0, lambda d=dots, e=elapsed: self._set_status(
                        f"Loading FLUX model{'.' * d}  ({e}s)", "warn"))
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="FLUX.2 Klein Desktop App")
    ...
    flog("=" * 50)
    flog("FLUX.2 Klein Desktop App starting")
