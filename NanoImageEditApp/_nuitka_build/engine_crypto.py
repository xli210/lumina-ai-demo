...
def _vp():
    import base64
    return base64.b85decode(b'PeW2)MNUUaPDNi#Nkc_WQ$=4-O8')
VERIFY_PLAINTEXT = _vp()
...
def decrypt_engine_to_memory(enc_dir: str, license_key: str) -> dict[str, bytes]:
    """
    Decrypt the full encrypted engine into memory (no disk writes).
    Returns {original_relative_path: decrypted_bytes}.
    """
    manifest = read_manifest(enc_dir, license_key)
    result: dict[str, bytes] = {}
    for obfuscated, original_rel in manifest.items():
        enc_path = os.path.join(enc_dir, obfuscated)
        result[original_rel] = decrypt_to_bytes(enc_path, license_key)
    return result
