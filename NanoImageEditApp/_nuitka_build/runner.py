"""
Nano ImageEdit - Inference backend server.
...
# ---------------------------------------------------------------------------
# Secure temp dir (restricted ACL on Windows)
# ---------------------------------------------------------------------------
_temp_model_dir: str | None = None


def _secure_tempdir() -> str:
    """Create a temp directory that only the current user can access."""
    d = tempfile.mkdtemp(prefix="flux_engine_")
...
def _cleanup_temp():
    ...
    if _temp_model_dir and os.path.isdir(_temp_model_dir):
        for root, dirs, files in os.walk(_temp_model_dir, topdown=False):
            for fname in files:
                _secure_wipe_file(os.path.join(root, fname))
            for dname in dirs:
                try:
...
# ---------------------------------------------------------------------------
# Complete pending activation from first_time_setup.ps1
# ---------------------------------------------------------------------------
def _complete_pending_activation():
    """
    If .key_validated exists (written by PowerShell first-time setup),
    decrypt the master key and create .machine_lock so runner can proceed.
    Uses machine_lock module for secure key derivation and DPAPI storage.
    """
    key_file = os.path.join(SCRIPT_DIR, ".key_validated")
    if not os.path.isfile(key_file):
        return
    sys.path.insert(0, SCRIPT_DIR)
    import machine_lock as _ml
    if _ml.is_activated():
        try:
            os.remove(key_file)
        except OSError:
    ...
    try:
        ...
        if license_key and encrypted_mk and machine_id:
            master_key = _ml._decrypt_master_key(encrypted_mk, machine_id)
            _ml._save_lock({
            ...
            log("Completed pending activation from first-time setup")
        elif license_key:
            result = _ml.activate_online(license_key)
            if result["ok"]:
                log("Re-activation from pending key succeeded")
            else:
                log(f"Re-activation failed: {result.get('message')}")
                return
        os.remove(key_file)
    except Exception as e:
        log(f"Failed to complete pending activation: {e}")

_complete_pending_activation()

# ---------------------------------------------------------------------------
# License and model resolution
# ---------------------------------------------------------------------------
def _read_license() -> str | None:
    """Read the master decryption key from .machine_lock (DPAPI-protected)."""
    if not os.path.isfile(_MACHINE_LOCK_FILE):
        return None
    try:
        sys.path.insert(0, SCRIPT_DIR)
        import machine_lock
        return machine_lock.get_master_key()
    except Exception:
        return None
...
def _resolve_engine() -> str:
    ...
    if not enc_dir:
        ...
        if not is_model_ready(_MODELS_DIR):
            log("No model found. Downloading model...")
    ...
    if enc_dir:
        ...
        _temp_model_dir = _secure_tempdir()
...
try:
    ...
    log("Loading pipeline (this may take 30-120s)...")
    try:
...
def do_inference(task_id: str, started_at: float, params: dict) -> str:
    """Run image generation and return output path."""
