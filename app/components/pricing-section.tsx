...
import { Check, ArrowRight, Download, Clock } from "lucide-react";
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {PRODUCTS.map((product) => {
            const isPaidWithTrial =
              product.priceInCents > 0 && (product.trialDays ?? 0) > 0;

            return (
              <div
                key={product.id}
                className={`glass-strong rounded-3xl p-6 border flex flex-col sm:p-8 ${
                  isPaidWithTrial
              ...
              >
                ...
                <p className="mb-6 text-xs text-muted-foreground">
                  {isPaidWithTrial
                    ? `${product.trialDays}-day free trial, then one-time payment`
                    : product.priceInCents === 0
